...
function postXPI (manifest, options) {
  ...
  return xpi(manifest, extend(options, { skipUpdateRDF: true })).then(function (xpiPath) {
		return when.promise(function(done) {
      fs.readFile(xpiPath, function(err, buffer) {
        var p = url.parse(postURL);
        var client = net.connect({ host: p.hostname, port: p.port }, function() {

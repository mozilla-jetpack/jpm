...
Client.prototype.downloadSignedFiles = function(signedFiles, options) {
  ...
  var self = this;
  var allDownloads = [];
  // Set some defaults so it looks like 0% before starting the download.
  var dataExpected = 0.00001;
  var dataReceived = 0;
  ...
  function showProgress() {
    ...
    var padding = Array(4 - amount.length).join(' ');
    options.stdout.write('\r' +
        'Downloading signed file(s): ' + padding + amount + '% ');
  }
  ...
  function download(fileUrl) {
    return when.promise(function(resolve, reject) {
      ...
      var fileName = path.join(process.cwd(),
                               path.basename(url.parse(fileUrl).path));
      ...
      req.on('response', function (data) {
        dataExpected += parseInt(data.headers['content-length']);
      });
  ...
  return when.all(allDownloads).then(function(downloadedFiles) {
    logger.log('Your signed add-on awaits:');

...
function runFirefox (manifest, options) {
  ...
  function handleProfile (profile) {
    ...
    function killFirefox () {
      firefox.kill();
      var tries = 0;
      // Windows release lock on files moments after the process ends
      while (fs.existsSync(profile) && tries < 1000) {
        try {
          fs.removeSync(profile);
        } catch (_) {}
        tries++;
      }
      if (fs.existsSync(profile)) {
        console.error("Failed to remove profile at " + profile);
      }
    }

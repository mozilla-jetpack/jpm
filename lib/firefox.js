...
function runFirefox (manifest, options) {
  ...
  if (!options.profile) {
    profile.createProfile(options).then(profileDeferred.resolve);
  }
  else {
    profileDeferred.resolve(options.profile);
  }

  function handleProfile (profilePath) {
    // Firefox takes -P if given a profile name and
    // -profile if given a path (starting with "/" or "./").
    if (profile.isProfileName(profilePath))
      args = args.concat(["-P", profilePath]);
    else
      args = args.concat(["-profile", profilePath]);

...
function getXpiInfoFromWebExtManifest(webExtManifest) {
  return when.promise(function(resolve, reject) {
    if (!webExtManifest.version) {
      return reject(new Error(
        "invalid web extension manifest: missing 'version'"));
    }
    if (!webExtManifest.applications ||
        !webExtManifest.applications.gecko ||
        !webExtManifest.applications.gecko.id) {

      return reject(new Error(
        "invalid web extension manifest: missing 'applications.gecko.id'"));
    }
    return resolve({
      manifest: webExtManifest,
      id: webExtManifest.applications.gecko.id,
      version: webExtManifest.version,
    });
  });
}
exports.getXpiInfoFromWebExtManifest = getXpiInfoFromWebExtManifest;
...
function getXpiInfo(xpiPath) {
  return extractXPI(xpiPath)
    .then(function(tmpXPI) {
      ...
      var manifestFile = path.join(tmpXPI.path, "manifest.json");
      var installRdfFile = path.join(tmpXPI.path, "install.rdf");

      if (fileExists(packageFile)) {
        getInfo = getXpiInfoFromManifest(require(packageFile));
      } else if (fileExists(manifestFile)) {
        getInfo = getXpiInfoFromWebExtManifest(require(manifestFile));

...
function toFileURI(filePath) {
  ...
  return "file:///" + path.join.apply(null, filePath).replace(path.sep, "/");
}
...
function includePrefs(id, options) {
  ...
  if (options.overload && (process.env.JETPACK_ROOT || typeof options.overload === "string")) {
    ...
    prefs["extensions.modules." + id + ".path."] = toFileURI(root);

    // Also need to set paths for shared loader so it can pick up changes.
    fs.readdirSync(root.join(path.sep)).forEach(function(entry) {
      prefs["modules.path." + entry + "."] = toFileURI(root.concat(entry))
    })
  }

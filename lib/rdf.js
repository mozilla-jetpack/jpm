...
function createRDF (manifest) {
  ...
  if (manifest.optionsURL) {
    var addonIdList = /^{([^}]+)}$|^@(.*)$/.exec(_id);
    // Like: resource://linkcheckerwce/data/page/option_page.html
    jetpackMeta["em:optionsURL"] = "resource://" + (addonIdList[1] || addonIdList[2]) + "/" + manifest.optionsURL;
    jetpackMeta["em:optionsType"] = 3;
  } else if (manifest.preferences) {
  ...
  if(manifest.localized_description){
    var   localizedDescription,
          langVoc;

    for(var langCode in manifest.localized_description){
      langVoc = manifest.localized_description[langCode];
      localizedDescription = {
        "em:locale": langCode
      };

      if (langVoc.title != void(0)) {
        localizedDescription["em:name"] = langVoc.title;
      }

      if (langVoc.description != void(0)) {
        localizedDescription["em:description"] = langVoc.description;
      }
...
function createUpdateRDF (manifest) {
  ...
  var description = {
    name: "Description",
    attrs:{ 
      "about": "urn:mozilla:"+getID(manifest)
    },
    children: []
  };
  ...
  if (engines.length) {
    engines.forEach(function (engine) {
      enginesDescription.children.push(createApplication(engine, manifest.engines[engine]));
    });
  }
  // Otherwise, assume default Firefox support
  else {
  ...
  enginesDescription.children.forEach(function (descriptionData, index) {
      if(Object.keys(descriptionData) == "em:targetApplication"){
        enginesDescription.children[index][Object.keys(descriptionData)].Description["em:updateLink"] = manifest.updateLink;
      }
  });

  ...
  if (!options.xpiPath) {
    ...
    return utils.getXpiInfo(options.xpiPath)
      .then(function(xpiInfo) {
        if (Object.keys(xpiInfo.manifest).length === 0) {
          // TODO: fix this once getXpiInfo() returns the right info.
          throw new Error(
            "This XPI does not have a package.json; " +
            "These XPIs are not supported yet");
        } else {
          return xpiInfo;
        }
      });

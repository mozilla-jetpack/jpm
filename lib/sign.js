...
function getXpiInfo(options) {
  ...
  if (!options.xpiPath) {
    ...
    return utils.extractXPI(options.xpiPath)
      .then(function(tmpXpi) {
        var manifest;
        var packagePath = path.join(tmpXpi.path, "package.json");
        try {
        ...
        if (manifest) {
          ...
          throw new Error("XPIs without a package.json are not supported yet");
        }
      });
  }
}

...
function sign (program, options, config) {
  ...
  return when.promise(function(resolve, reject) {
    ...
    var withManifest = config.getManifest({addonDir: program.addonDir, xpiPath: options.xpi})
      .then(function(manifest) {
        ...
        if (options.xpi) {
          ...
          return _.assign(xpiConfig, {xpiPath: options.xpi, removeUnsigned: false});
        } else {
          // Merge the program and sub-command options.
          var xpiOptions = _.assign({}, program, options);
          return config.createXPI(manifest, xpiOptions).then(function(xpiPath) {
            logger.log("Created XPI for signing: " + xpiPath);
            return _.assign(xpiConfig, {xpiPath: xpiPath, removeUnsigned: true});
          });
        }
      });
    ...
    return client.sign({
      ...
      if (xpiConfig.removeUnsigned) {
        client.debug('removing unsigned file', xpiConfig.xpiPath);
        config.removeFile(xpiConfig.xpiPath)
      }
      return result;
    });

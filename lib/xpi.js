var join = require("path").join;
var createRDF = require("./rdf");
var when = require("when");
var fs = require("fs-promise");
var getID = require("jetpack-id");
var validate = require("./validate");
var zip = require("./zip");
var console = require("./utils").console;
var hasAOMSupport = require("./utils").hasAOMSupport;

/**
 * Takes a manifest object (from package.json) and build options
 * object and zipz up the current directory into a XPI, copying
 * over default `install.rdf` and `bootstrap.js` if needed
 * and not yet defined. Returns a promise that resolves
 * upon completion.
 *
 * @param {Object} manifest
 * @param {Object} options
 *   - `xpiPath` Path where xpi should be saved. Used by `run`, `test` to indicate
 *               that the xpi saved in a temp directory.
 * @return {Promise}
 */

function xpi (manifest, options) {
  options = options || {};

  var dir = process.cwd();
  var xpiName = getID(manifest) + ".xpi";

  var xpiPath = join(options.xpiPath || dir, xpiName);
  var useFallbacks = !(options.forceAOM || hasAOMSupport(manifest));

  var needsInstallRDF;
  var needsBootstrapJS;

  return validate(dir)
    .then(checkNeedsFallbacks)
    .then(createFallbacks)
    .then(createZip)
    .then(removeFallbacks)
    .then(function () {
      return xpiPath;
    });

  function checkNeedsFallbacks() {
    if (!useFallbacks) {
      needsInstallRDF = false;
      needsBootstrapJS = false;
      return when.resolve();
    }
    if (useFallbacks && options.verbose) {
      console.log("Creating compatability bootstrap.js and install.rdf for xpi");
    }
    var rdfPath = join(dir, "install.rdf");
    var bsPath = join(dir, "bootstrap.js");
    return when.all([
      fs.exists(rdfPath).then(function(exists, a2) {
        if (exists) {
          console.warn("Using existing install.rdf. This file is usually auto-generated.");
        }
        needsInstallRDF = !exists;
      }),
      fs.exists(bsPath).then(function(exists) {
        if (exists) {
          console.warn("Using existing bootstrap.js. This file is usually auto-generated.");
        }
        needsBootstrapJS = !exists;
      })
    ]);
  }


  // Creates bootstrap.js/install.rdf -- will be removed once
  // AOM is updated
  function createFallbacks () {
    var rdfPath = join(dir, "install.rdf");
    var bsPath = join(dir, "bootstrap.js");
    var bootstrapSrc = join(__dirname, "../data/bootstrap.js");
    return when.all([
      needsInstallRDF ? fs.writeFile(rdfPath, createRDF(manifest)) : when.resolve(),
      needsBootstrapJS ? fs.copy(bootstrapSrc, bsPath) : when.resolve(),
    ]);
  }

  function createZip () {
    if (options.verbose) {
      console.log("Creating XPI...");
    }
    var start = Date.now();
    return zip(options, dir, xpiPath).then(function (result) {
      var diff = Date.now() - start;
      console.log("XPI created at " + xpiPath + " (" + diff + "ms)");
      return result;
    });
  }

  function removeFallbacks () {
    var rdfPath = join(dir, "install.rdf");
    var bsPath = join(dir, "bootstrap.js");
    return when.all([
      needsInstallRDF ? fs.remove(rdfPath) : when.resolve(),
      needsBootstrapJS ? fs.remove(bsPath) : when.resolve()
    ]);
  }
}
module.exports = xpi;

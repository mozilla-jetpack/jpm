...
function removeXPI(options) {
  return when.all(options.xpis.map(function(xpi) {
    return when.promise(function(resolve) {
      return fs.unlink(xpi).then(resolve);
    });
  }))
  .then(function() {
    return options;
...
function execute(manifest, options) {
  ...
  return setupPrefs(id, options).then(function() {
      return xpi(manifest, options);
    })
    .then(function(xpiPath) {
      options.xpis = options.xpis || [];
      options.xpis.push(xpiPath);
      return options;
    })
    .then(handleProfile)
...
function handleProfile(options) {
  // TODO: make sure this works, I don't see how extensions would be installed in this case.
  if (profile.isProfileName(options.profile)) {
    return when.resolve(options.profile);
  }
